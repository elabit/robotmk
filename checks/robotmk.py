...
STATES_NO = {"OK": 0, "WARNING": 1, "CRITICAL": 2, "UNKNOWN": 3}
...
def discover_robotmk(params, section):
    ...
    for root_suite in info_dict["suites"]:
        ...
        if "parsed" in root_suite:
            for discovered_item in root_suite["parsed"].discovered:
                service_description = add_svc_prefix(
                    discovered_item.name, root_suite, service_prefix
                )
                svc_labels_htmllog = []
                if (not is_piggyback_result and not suite_piggybackhost) or (
                    is_piggyback_result and suite_piggybackhost
                ):
                    # Displaying a hyperlink to the HTML logs on CMK services
                    # See Ref #VfHCJn in robotmk agent plugin
                    svc_labels_htmllog = assign_html_logs(
                        service_description, info_dict, root_suite, html_show_patterns
                    )
    ...
    if not is_piggyback_result:
        ...
        yield Service(item=svc_robotmk, parameters=params_dict, labels=svc_labels)
...
def check_robotmk(item, params, section):
    ...
    if item == svc_robotmk:
        ...
        # firstline
        if suites_total == 0:
            first_line.append("0 suites planned/executed (!). Check the configuration!")
            rc = max(rc, 1)
        else:
        ...
        if "runtime_total" in parsed_section["runner"]:
            ...
            try:
                if parsed_section["runner"]["execution_mode"] == "agent_serial":
                    ...
                    parsed_section["runner"]["execution_mode"] == "external"
                    and not parsed_section["runner"]["selective_run"]
                ):
                    cache_time = parsed_section["runner"]["cache_time"]
                ...
                pct_runtime_usage = round((100 / float(maxruntime)) * runner_runtime, 1)
        ...
        # 3. Execution mode
        first_line.append(
            "execution mode: %s" % parsed_section["runner"]["execution_mode"]
        )
        ...
        if client_version != ROBOTMK_VERSION:
            ...
            first_line.append(
                "Robotmk version %s (server and client)" % ROBOTMK_VERSION
            )

        # putting things together
        summary = ", ".join(first_line)
        details = "".join(out_lines) or None
        yield Result(state=State(rc), summary=summary, details=details)
    else:
            ...
            if "parsed" in root_suite:
                ...
                if html_exists:
                    ...
                    for host in runner_assigned_host:
                        ...
                        save_htmllog(
                            host_dir,
                            "%s_last_log.html" % root_suite["id"],
                            root_suite["htmllog"],
                        )
                        if root_suite["rc"] > 0:
                            save_htmllog(
                                host_dir,
                                "%s_last_error_log.html" % root_suite["id"],
                                root_suite["htmllog"],
                            )

                for discovered_item in root_suite["parsed"].discovered:
                    # Remove the prefix to get the original item name
                    item_without_prefix = strip_svc_prefix(
                        item, root_suite, service_prefix
                    )
...
def ignore_robot_item(root_suite, last_end, age):
    ...
    out = (
        "Result of suite '%s' is too old. " % root_suite["id"]
...
def get_svc_prefix_tplstring(itemname, root_suite, prefix):
    ...
    fmtstring = pattern_match(prefix, root_suite["parsed"].name, DEFAULT_SVC_PREFIX)
    template = Template(fmtstring)
    ret_prefix = template.safe_substitute(
        PATH=root_suite["path"],
        TAG=root_suite["tag"],
...
class RobotItem(object):
    ...
    def padstring(self):
        return (
            (int(RobotItem.discovery_setting.level) - self.lv_abs)
            * -1
    ...
    def __init__(self, xmlnode, lv_abs, lv_rel, parent, index=None):
        ...
        if xmlnode.attrib["name"] == "Add Monitoring Message":
            ...
            self.root_suite.robotmk_messages.append(data)
    ...
    def _get_subnodes(self, xmlnode):
        ...
        for nodetype in self.allowed_subnodes:
            for index, xmlsubnode in enumerate(xmlnode.findall(nodetype)):
                ...
                node = RobotClass(
                    xmlsubnode, self.lv_abs + 1, self.lv_rel + 1, self, index
                )
    ...
    def _eval_node_robotframework_status(self, check_params):
        if type(self) == RobotKeyword:
            ...
            if bool(check_params.get("show_kwmessages")) and not self.msg is None:
        ...
        self.result["result_robotframework"] = (
            ROBOT_NAGIOS_STATUS[self.status],
    ...
    def _set_node_padded_line(self, check_params):
        ...
        node_results = [
            NodeResult(check, self.result[check])
            for check in self.result_keys
        ...
        if len(node_messages) > 0:
            ...
            top_messages = [
                ...
                if (
                    node_result.resultuple[1]
                    and node_result.check != "result_cmk_runtime"
                )
                or (
                    node_result.resultuple[1]
                    and node_result.check == "result_cmk_runtime"
                    and node_result.resultuple[0] > 0
                )
            ]
            if top_messages:
    ...
    def _eval_node_cmk_runtime(self, check_params):
        ...
        if bool(runtime_threshold):
            ...
            # WARNING threshold
                ...
                nagios_status = 0
                if bool(
                    check_params.get("runtime_threshold", False).get(
                        "show_all_runtimes", False
                    )
                ):
    ...
    def _eval_total_result(self):
        ...
        subnodes_worststate = max(
            [s["worststate"] for s in self.subresults if self.subresults] or [0]
        )
        ...
        for subresult in self.subresults:
            if subresult["cmk_perfdata_list"] != None:
                try:
                    ...
                    self.result["cmk_perfdata_list"] = subresult["cmk_perfdata_list"]
        return
    ...
    def get_checkmk_result(self, node_top, check_params, depth_limit_inherited=None):
        ...
        # do the recursion
        self.subresults = []
        (descend_allowed, next_depth_limit) = self._descending_allowed(
            depth_limit_inherited, check_params
        )
        if descend_allowed:
        ...
        for host in runner_assigned_host:
            ...
            for log in ["last_log", "last_error_log"]:
                ...
                if re.match(pattern, svc_desc):
                    ...
                    svc_labels.append(ServiceLabel("robotmk/html_%s" % log, "yes"))
    return svc_labels
...
def check_stale_suites(suites):
    ...
    for root_suite in [s for s in suites if s["status"] != "fatal"]:
        ...
        if "end_time" in root_suite:
            ...
            if age.total_seconds() < root_suite["cache_time"]:
                # nonstale
                msg = "Suite '%s': (last execution end: %s, " % (
                    root_suite["id"],
                    last_end_fmt,
                )
                suites_nonstale.append(Suite(root_suite["id"], msg))
            else:
                # stale result
                ...
                msg = "(!!) Suite '%s': (last execution end: %s, " % (
                    ...
                    root_suite["cache_time"],
                    overdue_sec,
                )
                suites_stale.append(Suite(root_suite["id"], msg))
        else:
            ...
            if age.total_seconds() < root_suite["cache_time"]:
                # nonstale
                msg = "Suite '%s': (started 1st time at: %s, " % (
                    root_suite["id"],
                    last_start_fmt,
                )
                suites_nonstale.append(Suite(root_suite["id"], msg))
...
def html_to_text(html):
    ...
    html = re.sub(
        "<script.*?>|</script>|<style.*?>|<style>|<span.*?>|</span>|<a.*?>|</a>|<hr>",
        "",
        html,
    )
    html = re.sub("<br>|<p>", "\\n", html)
