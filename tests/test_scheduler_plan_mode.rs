...
async fn test_plan_mode() -> AnyhowResult<()> {
    ...
    let rcc_plan_result = results_dir.join("rcc_plan.json");
    assert!(
        rcc_plan_result.is_file(),
        "rcc_plan.json result file should exist"
    );

    let conda_plan_result = results_dir.join("conda_plan.json");
    assert!(
        !conda_plan_result.is_file(),
        "conda_plan.json result file should NOT exist (only rcc_plan was executed)"
    );
...
async fn test_plan_mode_no_result() -> AnyhowResult<()> {
    ...
    assert!(
        !rcc_plan_result.is_file(),
        "rcc_plan.json result file should NOT exist when --no-plan-result is specified"
    );
